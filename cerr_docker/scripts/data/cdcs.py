    ...
    for rec in response_content:
        ...
        if cert == "":
            ...
            response = requests.get(
                fetch_url, verify=True, cert=cert, auth=(username, password)
            )
        out = response.json()
...
def get_template(username="", password="", cert="", url="", title="", verbose=False):
    ...
    if verbose:
        print("status: Getting list of Templates...")
    if cert == "":
        response = requests.get(turl, verify=False, auth=(username, password))
    else:
        response = requests.get(turl, verify=True, cert=cert, auth=(username, password))
    ...
    if verbose:
        print(
            ...
            + str(response_code)
            + " - "
            + str(requests.status_codes._codes[response_code])
        )
    response_content = json.loads(response.text)
    for rec in response_content:
        if verbose:
    ...
    if verbose:
        print("status: getting specified Template..." + title)
    if cert == "":
        response = requests.get(turl, verify=False, auth=(username, password))
    else:
        response = requests.get(turl, verify=True, cert=cert, auth=(username, password))
...
def get_data_full(username="", password="", cert="", url="", id=""):
    ...
    if cert == "":
        response = requests.get(
            turl, params=data, verify=False, auth=(username, password)
        )
    else:
        response = requests.get(
            turl, params=data, verify=True, cert=cert, auth=(username, password)
        )
    response_code = response.status_code
    print(
        "Code:"
...
def upload_data(
    username="", password="", cert="", url="", filen="", template_id="", verbose=True
):
    """
    ...
    if cert == "":
        ...
        response = requests.post(
            turl, data=data, verify=True, cert=cert, auth=(username, password)
        )
    response_code = response.status_code
...
def upload_blob_file(username="", password="", cert="", url="", filen=""):
    ...
    if cert == "":
        response = requests.post(
            turl, files=xml_file, data=data, verify=False, auth=(username, password)
        )
    else:
        response = requests.post(
            turl,
...
def save_blob_to_file(dir="", username="", password="", cert="", url="", bid=""):
    ...
    if cert == "":
        response = requests.get(
            turl, verify=False, auth=(username, password), stream=True
        )
    else:
        response = requests.get(
            turl, verify=True, cert=cert, auth=(username, password), stream=True
        )
    response_code = response.status_code
    print(
        "Code:"
...
def get_data_by_title(
    username="", password="", cert="", url="", data_file_title="", verbose=False
):
    """
    Parameters:      username        - username on CDCS instance   (Note:  User must have admin privledges)
                     password        - password on CDCS instance
    ...
    if verbose:
        print("- status: Getting Data file..." + data_file_title)
...
def del_pid(
    ...
    if verbose:
        print("status: Deleteing Blob file..." + record)
    if verbose:
        print("Delete: " + turl)
    if cert == "":
        ...
        del_response = requests.delete(
            turl, verify=True, cert=cert, auth=(username, password)
        )
    if del_response.status_code == 204:
        if verbose:
            print("- status: " + record + " has been deleted.")
    else:
        print(
            "Error: a problem occurred when deleting the blobs with status code: "
            + str(del_response.status_code)
            + " - "
            + str(requests.status_codes._codes[del_response.status_code])
        )
...
def delete_data(username="", password="", cert="", url="", did="", verbose=False):
    ...
    if verbose:
        print("status: Deleteing Data file..." + did)
    if verbose:
        print("Delete: " + turl)
    if cert == "":
        del_response = requests.delete(turl, verify=False, auth=(username, password))
    ...
    if del_response.status_code == 204:
        if verbose:
            print("- status: " + did + " has been deleted.")
    else:
        print(
            "Error: a problem occurred when deleting the data file with status code: "
            + str(del_response.status_code)
            + " - "
            + str(requests.status_codes._codes[del_response.status_code])
        )
...
def dump_curator(username="", password="", url="", cert=""):
    ...
    if response_code == requests.codes.ok:
        ...
        print(
            "Error: Get Template Global failed with status code: "
            + str(response_code)
            + " - "
            + str(requests.status_codes._codes[response_code])
        )
    ...
    for rec in out:
        ...
        for templ in ver:
            ...
            return_code = subprocess.call(
                "mkdir ./schemas/" + title + str(counter), shell=True
            )
            pprint(templ)
            ...
            turl = url + template_upload_url
            print(
                "status: Getting Data files associated with template.."
                + title
                + ":"
                + templ
            )
            ...
            return_code = subprocess.call(
                "mkdir ./schemas/" + title + str(counter) + "/files", shell=True
            )

            if cert == "":
                response2 = requests.post(
                    turl, data, verify=False, auth=(username, password)
                )
            else:
                response2 = requests.post(
                    turl, data, verify=True, cert=cert, auth=(username, password)
                )
            ...
            if response_code == requests.codes.ok:
                ...
                for dataf in response_content["results"]:
                    ...
                    filename = (
                        "./schemas/" + title + str(counter) + "/files/" + dataf["title"]
                    )
                    ...
                    for bid in result:
                        if bid:
                            blobid = save_blob_to_file(
                                "./schemas/" + title + str(counter) + "/blobs/",
                                username,
                                password,
                                cert,
                                url,
    ...
    if response_code == requests.codes.ok:
        print(
            "- status: " + str(fcount) + "  Templates/files/blobs have been downloaded."
        )
    else:
        response.raise_for_status()
        raise Exception(
            "error: a problem occurred when downloadong the schema (Error ",
            response_code,
            ")",
        )
...
def load_curator(
    ...
    for rec in response_content:
        ...
        if schema_title == rec["title"]:
            ...
            template_upload_url = (
                "/rest/template-version-manager/" + template_id + "/version/"
            )
    ...
    if schema_filename != "":
        ...
        data = {"title": schema_title, "filename": newfile, "content": template_content}
        print("Load Template")
        turl = url + template_upload_url
        if cert == "":
            response = requests.post(
                turl, json=data, verify=False, auth=(username, password)
            )
        else:
            response = requests.post(
                turl, json=data, verify=True, cert=cert, auth=(username, password)
            )
        print("Post: " + turl)
        response_code = response.status_code
        ...
        if response_code == requests.codes.created:
            ...
            print(
                "status: Template has been uploaded with template_id = "
                + str(template_id)
            )
            if is_current == 1:
                ...
                if cert == "":
                    patch_response = requests.patch(
                        patch_url, verify=False, auth=(username, password)
                    )
                else:
                    patch_response = requests.patch(
                        patch_url, verify=True, cert=cert, auth=(username, password)
                    )
    ...
    for xml_filename in listdir(xml_dir):
        with codecs.open(
            join(xml_dir, xml_filename), mode="r", encoding="utf-8"
        ) as xml_file:
...
def load_blob(username="", password="", cert="", url="", dir="", bid=""):
    ...
    if cert == "":
        response = requests.get(
            blob_exists, verify=False, auth=(username, password), stream=True
        )
    else:
        response = requests.get(
            blob_exists, verify=True, cert=cert, auth=(username, password), stream=True
        )
...
def get_types_data(username="", pwd="", cert="", url=""):
    ...
    with open("types20.json", "wb") as fd:
        for chunk in response.iter_lines(
            chunk_size=512, decode_unicode=None, delimiter=None
        ):
            fd.write(chunk)
    response_code = response.status_code
    print("Resp: ")
    ...
    if response_code == requests.codes.ok:
        ...
        response.raise_for_status()
        raise Exception(
            "- error: a problem occurred when gettign the types20 (Error ",
            str(response_code)
            + " - "
            + str(requests.status_codes._codes[response_code]),
            ")",
...
def delete_all(username="", pwd="", url="", cert="", template=""):
    ...
    if response_code == requests.codes.ok:
        ...
        response.raise_for_status()
        raise Exception(
            "Error: a problem occurred when downloading the schem (Error ",
            str(response_code)
            + " - "
            + str(requests.status_codes._codes[response_code]),
            ")",
...
def delete_all_blobs(username="", password="", cert="", url=""):
    ...
    for item in response.json():
        ...
        if cert == "":
            del_response = requests.delete(
                turl, verify=False, auth=(username, password)
            )
        else:
            del_response = requests.delete(
                turl, verify=True, cert=cert, auth=(username, password)
        ...
        if del_response.status_code == 204:
            ...
            raise Exception(
                "- error: a problem occurred when deleting the blobs (Error ",
    ...
    if response_code == requests.codes.ok:
        ...
        response.raise_for_status()
        raise Exception(
            "- error: a problem occurred when deletng the blobs (Error ",
            response_code,
            ")",
        )
...
def del_all_xslts(username="", password="", cert="", url=""):
    ...
    for item in response.json():
        ...
        if del_response.status_code == 204:
            ...
            print(" error deleting: " + item["id"])
            raise Exception(
                "- error: a problem occurred when deleting the xslt (Error ",
                del_response.status_code,
                ")",
            )
...
def del_all_types(username="", password="", cert="", url=""):
    ...
    for item in response.json():
        ...
        if del_response.status_code == 204:
            ...
            print(" error deleting: " + item["id"])
            raise Exception(
                "- error: a problem occurred when deleting the types (Error ",
                del_response.status_code,
                ")",
            )
    ...
    if new_id == "":
        ...
        if response_code == requests.codes.ok:
            for rec in out:
                ...
                print("Get id =" + id)
                data_owner_url = (
                    "/rest/data/" + str(id) + "/change-owner/" + str(new_id)
                )
                ...
                if del_response.status_code == 200:
                    print(
                        "- status: "
                        + str(id)
                        + " has been changed, now owned by "
                        + new_user
                    )
                else:
                    print(
                        "Error: a problem occurred when changing the data file with status code: "
                        + str(del_response.status_code)
                        + " - "
                        + str(requests.status_codes._codes[del_response.status_code])
                    )
        ...
        if response_code == requests.codes.ok:
            for rec in out:
                ...
                print("Get id =" + id)
                data_owner_url = (
                    "/rest/blob/" + str(id) + "/change-owner/" + str(new_id)
                )
                durl = url + data_owner_url
                print("status: Changing Owner..." + new_user)
                print("Change: " + durl)
        ...
        if response_code == requests.codes.ok:
            ...
            response.raise_for_status()
            raise Exception(
                "error: a problem occurred when changing owner (Error ",
                response_code,
                ")",
            )
...
def export_data_by_template_json(
    username="", password="", cert="", url="", template_file_title=""
):
    """
...
def export_data_by_template_csv(
    username="", password="", cert="", url="", template_file_title=""
):
    """
...
def get_all_xslt(username="", password="", url="", cert=""):
    ...
    if response_code == requests.codes.ok:
        ...
        response.raise_for_status()
        raise Exception(
            "- error: a problem occurred when getting the xslt20 (Error ",
            response_code,
            ")",
        )
...
def replace_xslt(username="", password="", url="", cert="", xslt="", xslt_name=""):
    ...
    if cert == "":
        ...
        response = requests.get(
            xslt_url, verify=True, cert=cert, auth=(username, password)
        )
    print("Get Xslt ID:")
    response_code = response.status_code
    print(
    ...
    if cert == "":
        response = requests.patch(
            fetch_url, data, verify=False, auth=(username, password)
        )
    else:
        response = requests.patch(
            fetch_url, data, verify=True, cert=cert, auth=(username, password)
        )
    response_code = response.status_code
    response_content = json.loads(response.text)
    pprint(response_content)
...
def dump_registry(username="", password="", url="", cert=""):
    ...
    if response_code == requests.codes.ok:
        ...
        print(
            "Error: Registry Dump failed with status code: "
            + str(response_code)
            + " - "
            + str(requests.status_codes._codes[response_code])
        )
        response.raise_for_status()
        raise Exception(
            "Error: a problem occurred when dumping the registry (Error ",
            response_code,
            ")",
        )
    ...
    for rec in out:
        ...
        for templ in ver:
            ...
            process_content(
                "unpublished", username, password, url, title, templ, counter
            )
            script.write(
                "python3 load_registry.py "
                + username
...
def process_content(
    which="", username="", password="", url="", title="", templ="", counter=""
):
    template_upload_url = "/explore/common/rest/local-query"
    # template_upload_url = '/rest/data/query/'
    turl = url + template_upload_url
    ...
    return_code = subprocess.call(
        "mkdir ./schemas/" + title + str(counter) + "/" + which, shell=True
    )
    ...
    if response_code == requests.codes.ok:
        ...
        while (str(response_content["next"]) != "None") or (
            nfiles < response_content["count"]
        ):
            for dataf in response_content["results"]:
                nfiles = nfiles + 1
                print(
                    "--------------------------" + dataf["title"], " ---> ", str(nfiles)
                )
                pprint(dataf)
                print(
                    "--------------------------" + dataf["title"], " ---> ", str(nfiles)
...
def load_registry(
    username="", password="", url="", cert="", template="", files_dir="", published=""
):
    """
    ...
    for xml_filename in listdir(files_dir):
        ...
        with codecs.open(
            join(files_dir, xml_filename), mode="r", encoding="utf-8"
        ) as xml_file:
            xml_content = xml_file.read()
            xml_content = xml_content.replace("https://127.0.0.1", url)
            ...
            if response_code == requests.codes.created:
                if (publish_url != "") & (published == "1"):
                    response1 = requests.patch(
                        publish_url, verify=False, auth=(username, password)
                    )
                    ...
                    if response_code1 == requests.codes.ok:
                        print(
                            "status: "
                            + xml_filename
                            + " has been uploaded as Published."
                        )
                    else:
                        print(
                            "Error: Patch failed with status code: "
                            + str(response_code1)
                            + " - "
                            + str(requests.status_codes._codes[response_code1])
                        )
                        raise Exception(
                            "- error: a problem occurred when loading the file (Error ",
                            response_code1,
                            ")",
                        )
                else:
                    print(
                        "status: "
                        + xml_filename
                        + " has been uploaded as Un-Published."
                    )
